...
export async function middleware(request: NextRequest) {
  ...
  // Fix MaxelPay malformed URL: /membership&maxelpay_txn=...
  if (pathname.includes('membership') && pathname.includes('&')) {
    console.log('🔍 Detected malformed URL:', pathname);
    const url = request.nextUrl.clone();
    
    if (pathname.startsWith('/membership&')) {
      const queryPart = pathname.substring('/membership&'.length);
  ...
  if (request.nextUrl.pathname.startsWith('/admin') && !request.nextUrl.pathname.includes('/admin/unauthorized')) {
