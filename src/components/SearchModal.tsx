  ...
  const handleSearch = async () => {
    if (searchTerm.trim() && !isSearching) {
      ...
      if (selectedSection === 'writeups') {
        try {
          // Try Groq AI search first
          const groqRes = await fetch('/api/groq-search', {
            method: 'POST',
          ...
          if (groqRes.ok) {
            ...
            if (groqData.url) {
              window.location.href = groqData.url;
              onClose();
              return;
            } else if (groqData.suggestion) {
              setSuggestion(groqData.suggestion);
          ...
          console.log('Groq search failed, using fallback');
        }
        
        // Fallback only if Groq didn't provide suggestion
        try {
          ...
          if (htbMatch && htbMatch.id) {
            ...
            // Don't show noResults if we already have a suggestion from Groq
            if (!suggestion) {
              setNoResults(true);
      ...
      {isOpen && (
        ...
        >
          ...
          >
            ...
            {noResults && (
              <div className="mt-4 p-3 bg-red-500/20 border border-red-500/50 rounded-xl text-center">
                <p className="text-red-400 font-medium">{`No writeups found for "${searchTerm}"`}</p>
              </div>
            )}
            
            {suggestion && (
              <div className="mt-4 p-3 bg-cyber-blue/20 border border-cyber-blue/50 rounded-xl">
                <p className="text-cyber-blue font-medium text-sm">{suggestion}</p>
              </div>
            )}
