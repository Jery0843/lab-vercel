...
export async function GET(request: NextRequest) {
  try {
    ...
    const settingsQuery = await db.prepare('SELECT root_chat_enabled FROM chat_settings WHERE id = 1');
    const settings = await settingsQuery.first();
    if (settings && settings.root_chat_enabled === 0) {
    ...
    if (!member || member.tier_name !== 'Root Access') {
        ...
        SELECT m.*, 
          (SELECT json_group_array(json_object('emoji', emoji, 'count', count, 'users', users))
           FROM (SELECT emoji, COUNT(*) as count, json_group_array(user_email) as users
                 FROM root_chat_reactions WHERE message_id = m.id GROUP BY emoji))
          as reactions
        FROM root_chat_messages m WHERE m.created_at > ? ORDER BY m.created_at ASC
      `).bind(after);
    } else {
      messagesQuery = await db.prepare(`
        SELECT m.*, 
          (SELECT json_group_array(json_object('emoji', emoji, 'count', count, 'users', users))
           FROM (SELECT emoji, COUNT(*) as count, json_group_array(user_email) as users
                 FROM root_chat_reactions WHERE message_id = m.id GROUP BY emoji))
          as reactions
        FROM root_chat_messages m ORDER BY m.created_at ASC LIMIT 50
      `);
    }
    ...
    const pollsQuery = await db.prepare(`
      SELECT p.*, 
        (SELECT json_group_array(json_object('option_index', option_index, 'user_email', user_email))
         FROM root_chat_poll_votes WHERE poll_id = p.id) as votes
      FROM root_chat_polls p ORDER BY p.created_at DESC LIMIT 20
    `);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (type === 'message') {
      const insertQuery = await db.prepare('INSERT INTO root_chat_messages (user_name, user_email, message) VALUES (?, ?, ?) RETURNING *');
      ...
      await redis.lpush('chat:root:messages', JSON.stringify(redisMsg));
      await redis.ltrim('chat:root:messages', 0, 99); // Keep last 100
      
      return NextResponse.json({ success: true, message: newMessage });
    }

    if (type === 'reaction') {
      const insertQuery = await db.prepare('INSERT OR IGNORE INTO root_chat_reactions (message_id, user_email, emoji) VALUES (?, ?, ?)');
      await insertQuery.bind(messageId, email, reaction).run();
      return NextResponse.json({ success: true });
    }

    if (type === 'unreact') {
      const deleteQuery = await db.prepare('DELETE FROM root_chat_reactions WHERE message_id = ? AND user_email = ? AND emoji = ?');
      await deleteQuery.bind(messageId, email, reaction).run();
      return NextResponse.json({ success: true });
    }

    if (type === 'poll') {
      const insertQuery = await db.prepare('INSERT INTO root_chat_polls (user_name, user_email, question, options) VALUES (?, ?, ?, ?)');
      await insertQuery.bind(member.name, email, poll.question, JSON.stringify(poll.options)).run();
      return NextResponse.json({ success: true });
    }

    if (type === 'vote') {
      const insertQuery = await db.prepare('INSERT OR REPLACE INTO root_chat_poll_votes (poll_id, user_email, option_index) VALUES (?, ?, ?)');
    ...
    if (type === 'pin') {
      ...
      const updateQuery = await db.prepare('UPDATE root_chat_messages SET is_pinned = ? WHERE id = ?');
    ...
    if (type === 'delete') {
      ...
      const updateQuery = await db.prepare('UPDATE root_chat_messages SET is_deleted = 1, deleted_by = ?, message = "Message deleted" WHERE id = ? AND user_email = ?');
