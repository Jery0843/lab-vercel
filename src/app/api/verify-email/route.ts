...
async function sendOTP(email: string, otp: string, purpose: 'login' | 'writeup' = 'writeup'): Promise<boolean> {
  try {
    ...
    const subject = purpose === 'login' ? 'Your Login OTP' : 'Your OTP for Writeup Access';
    const message = purpose === 'login' 
      ? 'Use this OTP to complete your login:'
      : 'Use this OTP to access the writeup:';
...
export async function POST(request: NextRequest) {
  try {
    const { email, purpose = 'writeup' } = await request.json();
    ...
    // Send OTP via email
    const emailSent = await sendOTP(email, otp, purpose as 'login' | 'writeup');
    ...
    return NextResponse.json({
      success: true,
      message: `OTP sent to your email address for ${purpose}`
    });
