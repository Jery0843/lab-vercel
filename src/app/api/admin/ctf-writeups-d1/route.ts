...
import { sendWriteupNotification } from '@/trigger/email-jobs';
...
function dbToApiFormat(dbWriteup: any): CTFWriteupInput {
  return {
    ...
    accessTier: dbWriteup.access_tier || 'Both',
...
function apiToDbFormat(apiWriteup: CTFWriteupInput): CTFWriteup {
  ...
  return {
    ...
    access_tier: apiWriteup.accessTier || 'Both',
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (created.status === 'Completed' && created.writeup) {
      await sendWriteupNotification.trigger({
        writeupTitle: created.title,
        platform: created.ctf_name,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (updated.status === 'Completed' && updated.writeup && 
      ...
      await sendWriteupNotification.trigger({
        writeupTitle: updated.title,
        platform: updated.ctf_name,
