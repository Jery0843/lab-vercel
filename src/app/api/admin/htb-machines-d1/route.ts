...
import { sendMachineNotification } from '@/trigger/email-jobs';
...
function dbToApiFormat(dbMachine: any): HTBMachineInput {
  return {
    ...
    accessTier: dbMachine.access_tier || 'Both',
...
function apiToDbFormat(apiMachine: HTBMachineInput): HTBMachine {
  return {
    ...
    access_tier: apiMachine.accessTier || 'Both',
...
export async function POST(request: NextRequest) {
  try {
    ...
    await sendMachineNotification.trigger({
      machineName: created.name,
      machineOs: created.os,
      machineDifficulty: created.difficulty
    });
