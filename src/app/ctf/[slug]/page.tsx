...
export default function CTFWriteupPage() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (foundWriteup) {
            ...
            // Check access for this writeup
            checkWriteupAccess(foundWriteup.id);
          } else {
  ...
  const checkWriteupAccess = async (writeupId: string) => {
    try {
      setCheckingAccess(true);
      const response = await fetch('/api/check-writeup-access', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ writeupId, type: 'ctf' })
      });
  ...
  if (writeup && accessCheck && !accessCheck.hasAccess && !checkingAccess) {
    return (
      <Layout>
        <WriteupAccessDenied
          title={writeup.title}
...
function CTFWriteupContent({ writeup }: { writeup: CTFWriteup }) {
  ...
  const [writeupContent, setWriteupContent] = useState(writeup.writeup || '');
          ...
          <motion.main initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6, duration: 0.5 }} className="lg:col-span-8">
            <div className={`rounded-2xl p-4 md:p-6 lg:p-8 min-h-[60vh] ${writeup.writeup && writeup.writeup.length > 10000 ? 'backdrop-blur-lg bg-black/80 light:bg-white/85 border border-white/20' : 'backdrop-blur-sm bg-black/20 light:bg-white/30 border border-white/10'}`} style={{ animation: 'panelLoad 1s ease-out forwards' }}>
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-cyber-green font-cyber">CHALLENGE BRIEFING // WRITEUP</h2>
                {writeup.writeup && (
                  <LanguageSelector 
