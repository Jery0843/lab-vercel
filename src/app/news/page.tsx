...
const News = () => {
  ...
  const [trendingVectors, setTrendingVectors] = useState<TrendingVector[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchCVEs = async (forceRefresh = false) => {
      try {
        ...
        // Fetch trending vectors
        if (data.length > 0) {
          fetchTrendingVectors(data);
        }
      } catch (error) {
  ...
  const fetchTrendingVectors = async (cveData: CVEItem[]) => {
    setLoadingTrends(true);
    try {
      const response = await fetch('/api/trending-vectors', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ cves: cveData })
      });
      const data = await response.json();
      setTrendingVectors(data.vectors || []);
    } catch (error) {
      console.error('Failed to fetch trending vectors:', error);
    } finally {
      setLoadingTrends(false);
    }
  };

  const generateWriteupTemplate = async (cve: CVEItem) => {
    setGeneratingTemplate(true);
    setSelectedCVE(cve.id);
    try {
      const response = await fetch('/api/generate-writeup-template', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          cveId: cve.id,
      ...
      a.href = url;
      a.download = `${cve.id}-writeup-template.md`;
      a.click();
  ...
  return (
    <Layout>
      <div className="py-6 sm:py-8 lg:py-12">
        ...
        {/* AI Trending Attack Vectors Dashboard */}
        {activeTab === 'cves' && trendingVectors.length > 0 && (
          <div className="max-w-6xl mx-auto mb-8 px-4">
            <div className="rounded-2xl backdrop-blur-sm bg-gradient-to-r from-purple-900/20 to-pink-900/20 border border-purple-500/30 p-6">
              <h3 className="text-xl font-cyber font-bold text-purple-400 mb-4 flex items-center">
                <FaShieldAlt className="mr-2" />
                AI-Detected Trending Attack Vectors
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {trendingVectors.map((vector, idx) => (
                  <div key={idx} className="bg-black/40 border border-purple-500/20 rounded-lg p-4">
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="font-bold text-white">{vector.name}</h4>
                      <span className={`px-2 py-1 text-xs rounded ${getSeverityColor(vector.severity)}`}>
                        {vector.severity}
                      </span>
                    </div>
                    <p className="text-sm text-gray-400 mb-2">{vector.description}</p>
                    <div className="text-xs text-purple-400 font-bold">{vector.count} CVEs detected</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="max-w-lg mx-auto mb-6 sm:mb-8 px-4">
          ...
          <div className="flex items-center justify-center space-x-2 mt-2 text-xs text-gray-400">
            ...
            <span className="text-center">
              {activeTab === 'cves' 
                ? 'Showing latest CVEs from last 3 days (sorted by severity)' 
                ...
                >
                  ...
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-3 sm:space-y-0">
                    ...
                    <div className="flex items-center space-x-2">
                      <button
                        onClick={() => generateWriteupTemplate(cve)}
                        disabled={generatingTemplate && selectedCVE === cve.id}
                        className="flex items-center space-x-1 bg-purple-600 hover:bg-purple-700 text-white px-3 py-1 rounded text-xs font-bold transition-colors disabled:opacity-50"
                      >
                        <FaNewspaper className="text-xs" />
                        <span>{generatingTemplate && selectedCVE === cve.id ? 'Generating...' : 'AI Template'}</span>
                      </button>
